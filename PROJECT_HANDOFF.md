...
- 当前分支：`main`
- 当前提交：`f061c44`（实现文件存储可靠性）
- 文档更新日期：2026-09-05
- 当前工作区包含尚未提交的重复渲染修复和文档同步：新建、导入文件时只渲染一次文件列表，PreviewArea 只在阅读模式真正显示时解析 Markdown
- `npm run build` 已通过：Vite 8.1.4 共转换 73 个模块
- 当前阶段：核心文件业务、DocumentArea 阅读体验、Workspace 交互式布局、亮暗主题、文件存储可靠性和重复渲染精简已经完成；导入流程增强和导出文件名清理当前不计划，文件重命名暂缓，后续优先处理工程配置和代码质量
